...
class Program
{
    static void Main()
    {
        ...
        IStoreService storeService = new StoreService(userRepository, productRepository, purchaseRepository);
    ...
    // Виведення інформації про продукт за його id
   static void DisplayProductInfoById(IDatabase database)
{
    ...
    // Виведення інформації про всіх користувачів та їхню історію
    ...
    static void MakePurchase(IDatabase database)
    {
        ...
        // Створення нового користувача, якщо введений ім'я не знайдено
        if (user == null)
        {
            user = new User
            {
                Username = username,
                Balance = 0 // Баланс для нового користувача
            };
    ...
    // Поповнення або створення користувача для подальших операцій
